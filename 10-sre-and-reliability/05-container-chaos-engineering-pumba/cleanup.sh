#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Resource Teardown for Mini-Project 10-05
# ==============================================================================
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --all, --purge-images   Remove Pumba and microservices Docker container images"
...
echo "======================================================================"
echo "  🧹 Cleaning Up Container Chaos Engineering Stack"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
if [[ -n "$COMPOSE_CMD" ]] && [[ -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
    ...
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Containers 'api-gateway', 'payment-service-1', 'payment-service-2' stopped and removed."
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Network 'sre-chaos-net' removed."
else
    if command -v docker >/dev/null 2>&1; then
        docker rm -f api-gateway payment-service-1 payment-service-2 >/dev/null 2>&1 || true
        docker network rm sre-chaos-net >/dev/null 2>&1 || true
...
if [ "$PURGE_IMAGES" = true ]; then
    echo -e "\n${CLR_YELLOW}▶ [2/3] Purging Pumba, Gateway, and Payment Service container images...${CLR_RESET}"
    if command -v docker >/dev/null 2>&1; then
        docker rmi -f sre-api-gateway:latest sre-payment-service:latest gaiaadm/pumba:latest python:3.11-slim >/dev/null 2>&1 || true
