#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Resource Teardown for Mini-Project 10-03
# ==============================================================================
# Stops and removes Docker containers, networks, temporary logs, and optionally
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --all, --purge-images   Remove Executor and Mock Services Docker container images"
...
echo "======================================================================"
echo "  🧹 Cleaning Up Automated Incident Runbook Executor Stack"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
if [[ -n "$COMPOSE_CMD" ]] && [[ -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
    ...
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Containers 'runbook-executor' and 'mock-services' stopped and removed."
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Network 'sre-runbook-net' removed."
else
    if command -v docker >/dev/null 2>&1; then
        docker rm -f runbook-executor mock-services >/dev/null 2>&1 || true
        docker network rm sre-runbook-net >/dev/null 2>&1 || true
...
if [ "$PURGE_IMAGES" = true ]; then
    echo -e "\n${CLR_YELLOW}▶ [2/3] Purging Executor and Mock Services container images...${CLR_RESET}"
    if command -v docker >/dev/null 2>&1; then
        docker rmi -f sre-runbook-executor:latest sre-mock-services:latest python:3.11-slim >/dev/null 2>&1 || true
...
rm -f "$SCRIPT_DIR/execution_audit.json"
