#!/usr/bin/env bash
# ==============================================================================
# Teardown & Resource Cleanup: Linux Auditd Security Event Log Analysis
# ==============================================================================
set -e
...
echo -e "${CLR_CYAN}${CLR_BOLD}  🧹 Cleaning Up Linux Auditd SIEM Resources${CLR_RESET}"
...
docker rm -f audit-siem-dashboard audit-siem-shipper >/dev/null 2>&1 || true
echo -e "  [${CLR_GREEN}OK${CLR_RESET}] All containers removed."

# Volume cleanup
if docker volume ls -q | grep -q "^audit_logs_data$"; then
    docker volume rm -f audit_logs_data >/dev/null 2>&1 || true
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Volume 'audit_logs_data' deleted."
fi

# Network cleanup
if docker network ls --format '{{.Name}}' | grep -q "^audit-siem-net$"; then
    docker network rm audit-siem-net >/dev/null 2>&1 || true
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Network 'audit-siem-net' removed."
fi
...
if [ "$PURGE_IMAGES" = true ]; then
    ...
    docker rmi -f mini-proj-09-10-siem:local mini-proj-09-10-shipper:local >/dev/null 2>&1 || true
