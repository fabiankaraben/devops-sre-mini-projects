#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Resource Cleanup for Project 10
# ==============================================================================
# Purges local Docker containers, images, volumes, and destroys AWS
# infrastructure (Route 53, Primary/Secondary S3 buckets, IAM) via Terraform.
# ==============================================================================
...
echo "======================================================================"
echo "  🧹 Cleaning Up Multi-Region Disaster Recovery Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
# ------------------------------------------------------------------------------
echo -e "${CLR_YELLOW}▶ [1/4] Terminating background Python processes & test runners...${CLR_RESET}"
pkill -f "python3.*failover_gateway.py" 2>/dev/null || true
pkill -f "python3.*app/main.py" 2>/dev/null || true
pkill -f "dr_failover_simulator.py" 2>/dev/null || true
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    ...
    CONTAINERS=$(docker ps -a --filter "name=aws-us-east-1" --filter "name=aws-us-west-2" --filter "name=aws-route53" -q 2>/dev/null || true)
    ...
    IMAGES=$(docker images --filter "reference=*multi-region-dr*" -q 2>/dev/null || true)
    if [[ -n "$IMAGES" ]]; then
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [3/4] Destroying Terraform Multi-Region Cloud Infrastructure...${CLR_RESET}"
IAC_BIN=""
...
if [[ -n "$IAC_BIN" ]] && [[ -f "$SCRIPT_DIR/terraform.tfstate" ]]; then
    ...
    if [[ "$RESOURCE_COUNT" -gt 0 ]]; then
        echo -e "  Found ${RESOURCE_COUNT} managed AWS cloud resources across regions. Destroying with '$IAC_BIN destroy'..."
...
rm -f "$SCRIPT_DIR/test_report.json" \
      "$SCRIPT_DIR/failover_report.json" \
      "$SCRIPT_DIR/response.json" \
      "$SCRIPT_DIR/tfplan" \
      "$SCRIPT_DIR"/*.log \
      "$SCRIPT_DIR"/app/*.pyc \
      "$SCRIPT_DIR"/router/*.pyc \
      2>/dev/null || true

rm -rf "$SCRIPT_DIR/app/__pycache__" \
       "$SCRIPT_DIR/router/__pycache__" \
