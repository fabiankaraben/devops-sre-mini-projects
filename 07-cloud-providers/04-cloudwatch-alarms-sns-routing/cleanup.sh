#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Resource Teardown for Mini-Project 07-04
# ==============================================================================
# Destroys provisioned CloudWatch alarms, log groups, metric filters, and SNS
# topics, terminates webhook receivers, stops LocalStack containers, and purges state.
# ==============================================================================
...
PURGE_STATE=false
LOCALSTACK_CONTAINER="localstack-monitoring-demo"
...
echo "======================================================================"
echo "  🧹 Cleaning Up CloudWatch Alarms & SNS Routing Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"

# ------------------------------------------------------------------------------
# 1. Terminate Background Webhook Receiver Server
# ------------------------------------------------------------------------------
echo -e "${CLR_YELLOW}▶ [1/4] Terminating background webhook receiver processes...${CLR_RESET}"
pkill -f "webhook_receiver.py" >/dev/null 2>&1 || true
echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Webhook processes stopped."

# ------------------------------------------------------------------------------
# 2. Destroy Terraform Cloud Infrastructure
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [2/4] Destroying Terraform / OpenTofu Infrastructure...${CLR_RESET}"
IAC_BIN=""
...
if [[ -n "$IAC_BIN" ]] && [[ -f "$SCRIPT_DIR/terraform.tfstate" ]]; then
    ...
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] CloudWatch alarms, log groups, and SNS topics destroyed."
else
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [3/4] Stopping LocalStack container (if running)...${CLR_RESET}"
...
find "$SCRIPT_DIR" -type f \( \
    ...
    -name "test_webhook_events.json" -o \
    -name "*.pyc" \
