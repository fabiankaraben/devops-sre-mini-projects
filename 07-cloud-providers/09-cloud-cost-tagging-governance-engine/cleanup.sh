#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Resource Cleanup for Project 09
# ==============================================================================
# Purges local Docker containers, images, volumes, and destroys AWS
# infrastructure (Lambda, EventBridge, S3 audit bucket, SNS) via Terraform.
# ==============================================================================
...
echo "======================================================================"
echo "  🧹 Cleaning Up Cloud Cost Governance & Tag Compliance Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
# ------------------------------------------------------------------------------
echo -e "${CLR_YELLOW}▶ [1/4] Terminating background Python engines & test runners...${CLR_RESET}"
pkill -f "python3.*lambda_function.py" 2>/dev/null || true
pkill -f "governance_simulator.py" 2>/dev/null || true
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    ...
    CONTAINERS=$(docker ps -a --filter "name=finops-tag-governance" -q 2>/dev/null || true)
    ...
    IMAGES=$(docker images --filter "reference=*governance-engine*" -q 2>/dev/null || true)
    if [[ -n "$IMAGES" ]]; then
...
if [[ -n "$IAC_BIN" ]] && [[ -f "$SCRIPT_DIR/terraform.tfstate" ]]; then
    ...
    if [[ "$RESOURCE_COUNT" -gt 0 ]]; then
        echo -e "  Found ${RESOURCE_COUNT} managed AWS cloud resources in state. Destroying with '$IAC_BIN destroy'..."
...
rm -f "$SCRIPT_DIR/test_report.json" \
      "$SCRIPT_DIR/compliance_report.json" \
      "$SCRIPT_DIR/compliance_report.html" \
      "$SCRIPT_DIR/lambda_function.zip" \
      "$SCRIPT_DIR/tfplan" \
      "$SCRIPT_DIR"/*.log \
      "$SCRIPT_DIR"/engine/*.pyc \
      2>/dev/null || true

rm -rf "$SCRIPT_DIR/engine/__pycache__" \
       "$SCRIPT_DIR/__pycache__" \
       "$SCRIPT_DIR/custodian-out" \
       "$SCRIPT_DIR/.custodian" \
       2>/dev/null || true
