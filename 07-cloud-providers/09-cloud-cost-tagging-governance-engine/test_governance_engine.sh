#!/usr/bin/env bash
# ==============================================================================
# test_governance_engine.sh - Master Test Runner for Project 09
# ==============================================================================
# Validates Python syntax, Bash scripts, Terraform IaC manifests, and executes
# the end-to-end FinOps Cost Governance and Tag Compliance simulation suite.
# ==============================================================================
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --verbose|-v)
            ...
            ;;
        --help|-h)
            echo "Usage: ./test_governance_engine.sh [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --docker       Run automated Docker Compose container test"
            echo "  --live         Run tests against live AWS Lambda function from Terraform"
        ...
        *)
            ...
            echo "Run ./test_governance_engine.sh --help for usage."
            exit 1
            ;;
    esac
done
...
echo "======================================================================"
echo "  🏷️ Cloud Cost Governance & Tag Compliance Test Runner"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
for py_file in "$SCRIPT_DIR/engine/lambda_function.py" "$SCRIPT_DIR/governance_simulator.py"; do
...
for sh_file in "$SCRIPT_DIR/test_governance_engine.sh" "$SCRIPT_DIR/cleanup.sh" "$SCRIPT_DIR/provision_untagged_resources.sh"; do
...
# ------------------------------------------------------------------------------
# 4. Execute Offline FinOps Governance Simulation Suite
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [4/5] Executing Offline FinOps Governance Simulator Tests...${CLR_RESET}"
...
if python3 "$SCRIPT_DIR/governance_simulator.py" "${SIM_ARGS[@]}"; then
...
if [[ "$RUN_DOCKER" == true ]] || (command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1 && [[ "$RUN_LIVE" == false ]]); then
    echo "  Starting local Docker Compose Governance Engine container..."
    docker compose -f "$SCRIPT_DIR/docker-compose.yml" up -d --build >/dev/null 2>&1 || true

    echo "  Waiting for Governance Engine container to become healthy (up to 10s)..."
    ENGINE_READY=false
    for _ in {1..10}; do
    ...
    if [[ "$ENGINE_READY" == true ]]; then
        echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Local Governance Engine is active at http://localhost:8080"
        echo "  Running test resource provisioning & compliance scan..."
        "$SCRIPT_DIR/provision_untagged_resources.sh" --url "http://localhost:8080"
    else
        echo -e "  [${CLR_YELLOW}INFO${CLR_RESET}] Docker container not reachable, proceeding with mock validation."
        "$SCRIPT_DIR/provision_untagged_resources.sh" --mock
    fi
    ...
    echo "  Running live test against AWS Lambda function..."
...
echo -e "${CLR_GREEN}${CLR_BOLD}  🎉 All Cloud Cost Governance & Tag Compliance Tests Passed!${CLR_RESET}"
