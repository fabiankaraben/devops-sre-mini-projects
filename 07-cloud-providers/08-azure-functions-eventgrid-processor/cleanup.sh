#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Resource Cleanup for Project 08
# ==============================================================================
# Purges local Docker containers, images, volumes, Azurite emulator storage,
# and cleans up Microsoft Azure infrastructure (via Terraform).
# ==============================================================================
...
echo "======================================================================"
echo "  🧹 Cleaning Up Azure Event Grid & Blob Processor Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
# ------------------------------------------------------------------------------
echo -e "${CLR_YELLOW}▶ [1/4] Terminating background Python functions & test runners...${CLR_RESET}"
pkill -f "python3.*function_app.py" 2>/dev/null || true
pkill -f "upload_test_blobs.py" 2>/dev/null || true
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    ...
    CONTAINERS=$(docker ps -a --filter "name=azure-function" --filter "name=azure-storage-azurite" -q 2>/dev/null || true)
    ...
    IMAGES=$(docker images --filter "reference=*azure-eventgrid*" -q 2>/dev/null || true)
    if [[ -n "$IMAGES" ]]; then
...
if [[ -n "$IAC_BIN" ]] && [[ -f "$SCRIPT_DIR/terraform.tfstate" ]]; then
    ...
    if [[ "$RESOURCE_COUNT" -gt 0 ]]; then
        echo -e "  Found ${RESOURCE_COUNT} managed Azure cloud resources in state. Destroying with '$IAC_BIN destroy'..."
        "$IAC_BIN" destroy -auto-approve "$SCRIPT_DIR"
        echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Azure infrastructure destroyed."
    else
...
rm -f "$SCRIPT_DIR/test_report.json" \
      "$SCRIPT_DIR/test_upload_results.json" \
      "$SCRIPT_DIR/tfplan" \
      "$SCRIPT_DIR"/*.log \
      "$SCRIPT_DIR"/function/*.pyc \
      2>/dev/null || true

rm -rf "$SCRIPT_DIR/function/__pycache__" \
       "$SCRIPT_DIR/__pycache__" \
       "$SCRIPT_DIR/test_blobs_temp" \
       "$SCRIPT_DIR/__blobstorage__" \
       "$SCRIPT_DIR/__queuestorage__" \
       "$SCRIPT_DIR/__tablestorage__" \
       2>/dev/null || true
