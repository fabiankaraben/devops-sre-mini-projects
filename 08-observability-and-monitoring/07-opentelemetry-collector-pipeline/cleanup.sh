#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Resource Teardown for Mini-Project 08-07
# ==============================================================================
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --all, --purge-images   Remove pipeline container images"
...
echo "======================================================================"
echo "  🧹 Cleaning Up OpenTelemetry Collector Pipeline Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
if [[ -n "$COMPOSE_CMD" ]] && [[ -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
    ...
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Containers 'otel-collector-pipeline', 'sample-app-pipeline', 'prometheus-pipeline', 'jaeger-pipeline' stopped and removed."
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Bridge network 'collector-pipeline-net' removed."
else
    if command -v docker >/dev/null 2>&1; then
        docker rm -f otel-collector-pipeline sample-app-pipeline prometheus-pipeline jaeger-pipeline >/dev/null 2>&1 || true
        docker network rm collector-pipeline-net >/dev/null 2>&1 || true
...
if [ "$PURGE_IMAGES" = true ]; then
    echo -e "\n${CLR_YELLOW}▶ [2/3] Purging pipeline Docker images...${CLR_RESET}"
    if command -v docker >/dev/null 2>&1; then
        docker rmi -f \
