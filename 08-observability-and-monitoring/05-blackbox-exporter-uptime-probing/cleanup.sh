#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Resource Teardown for Mini-Project 08-05
# ==============================================================================
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --all, --purge-images   Remove Blackbox Exporter, Prometheus, and Mock Targets images"
...
echo "======================================================================"
echo "  🧹 Cleaning Up Blackbox Exporter & Probing Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
if [[ -n "$COMPOSE_CMD" ]] && [[ -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
    ...
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Containers 'prometheus-server', 'blackbox-exporter', 'mock-targets' removed."
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Network 'probing-stack-net' removed."
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Named volume 'prometheus_blackbox_data' deleted."
else
    if command -v docker >/dev/null 2>&1; then
        docker rm -f prometheus-server blackbox-exporter mock-targets >/dev/null 2>&1 || true
        docker volume rm prometheus_blackbox_data >/dev/null 2>&1 || true
        docker network rm probing-stack-net >/dev/null 2>&1 || true
...
if [ "$PURGE_IMAGES" = true ]; then
    echo -e "\n${CLR_YELLOW}▶ [2/3] Purging Blackbox Exporter, Prometheus, and Mock Targets images...${CLR_RESET}"
    if command -v docker >/dev/null 2>&1; then
        docker rmi -f mini-proj-08-05-prometheus:local mini-proj-08-05-blackbox:local mini-proj-08-05-mock-targets:local >/dev/null 2>&1 || true
