#!/usr/bin/env bash
# ==============================================================================
# test_priority_pipeline.sh - End-to-End Automated Test Suite for Mini-Project 15
# ==============================================================================
# Verifies:
#   1. Tool prerequisites (Docker, kubectl, Go)
#   2. Multi-stage Docker image build for priority-workload
#   3. Image footprint (< 25MB) and non-root UID 10001 security validation
#   4. Declarative PriorityClass, LimitRange & ResourceQuota manifests (verify_priority_preemption.sh)
#   5. Starvation and priority preemption test (starvation_test.sh)
...
cleanup_e2e() {
    local pids
    pids=$(pgrep -f "port-forward.*priority-workload" 2>/dev/null || true)
    if [[ -n "$pids" ]]; then
...
echo "======================================================================"
echo "  🚀 Priority Classes, Preemption & Quotas End-to-End Test Suite"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [2/6] Building Priority Workload Container Image...${CLR_RESET}"
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    if docker build -t priority-workload:v1.0.0 "${SCRIPT_DIR}/app" > "$TMP_DIR/docker_build.log" 2>&1; then
        record_test "03" "Docker multi-stage build (priority-workload:v1.0.0)" 0 "Built successfully"
    else
        record_test "03" "Docker multi-stage build" 1 "Build failed, check $TMP_DIR/docker_build.log"
    fi

    IMG_SIZE_MB=$(docker image inspect priority-workload:v1.0.0 --format='{{.Size}}' 2>/dev/null | awk '{printf "%.2f", $1/1024/1024}' || echo "0")
    ...
    USER_ID=$(docker image inspect priority-workload:v1.0.0 --format='{{.Config.User}}' 2>/dev/null || echo "")
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [3/6] Running Priority & Quota Policy Validation...${CLR_RESET}"
if "${SCRIPT_DIR}/verify_priority_preemption.sh"; then
    record_test "06" "Priority & Quota policy validation script (verify_priority_preemption.sh)" 0 "All checks passed"
else
    record_test "06" "Priority & Quota policy validation script" 1 "Validation checks failed"
fi

# ------------------------------------------------------------------------------
# Test 4: Run Starvation and Preemption Test
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [4/6] Running Starvation & Preemption Test...${CLR_RESET}"
if "${SCRIPT_DIR}/starvation_test.sh"; then
    record_test "07" "Starvation and preemption test (starvation_test.sh)" 0 "Preemption test passed"
else
    record_test "07" "Starvation and preemption test" 1 "Preemption test encountered errors"
fi
...
if "${SCRIPT_DIR}/cleanup.sh" >/dev/null 2>&1; then
    record_test "08" "Cleanup script execution (cleanup.sh)" 0 "All priority/quota resources purged"
else
...
if [[ "$FAILED_TESTS" -eq 0 ]]; then
    echo -e "  ${CLR_GREEN}${CLR_BOLD}🎉 ALL PRIORITY & PREEMPTION TESTS PASSED (${PASSED_TESTS}/${TOTAL_TESTS})${CLR_RESET}"
    echo "======================================================================"
    exit 0
else
