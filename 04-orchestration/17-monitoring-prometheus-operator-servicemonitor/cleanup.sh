#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Cleanup Script for Mini-Project 17
# ==============================================================================
# Purges:
#   1. Background port-forward processes targeting monitored-app or prometheus
#   2. Kubernetes namespaces: monitoring and production-apps
#   3. Prometheus Operator CRDs (ServiceMonitor, PodMonitor, PrometheusRule, Prometheus)
#   4. Local Docker images (monitored-app:v1.0.0) and test containers
...
echo "======================================================================"
echo "  🧹 Cleaning Up Prometheus Operator & Monitoring Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"

# 1. Terminate background port-forward tunnels
echo -e "${CLR_YELLOW}▶ [1/4] Terminating background port-forward processes...${CLR_RESET}"
PIDS=$(pgrep -f "port-forward.*(monitored-app|prometheus|grafana)" 2>/dev/null || true)
if [[ -n "$PIDS" ]]; then
...
echo -e "\n${CLR_YELLOW}▶ [2/4] Purging Kubernetes Namespaces & Prometheus CRDs...${CLR_RESET}"
if command -v kubectl >/dev/null 2>&1 && kubectl cluster-info >/dev/null 2>&1; then
    kubectl delete namespace monitoring production-apps --ignore-not-found=true --timeout=60s || true
    kubectl delete crd servicemonitors.monitoring.coreos.com \
        podmonitors.monitoring.coreos.com \
        prometheusrules.monitoring.coreos.com \
        prometheuses.monitoring.coreos.com --ignore-not-found=true >/dev/null 2>&1 || true
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Namespaces and Prometheus Operator CRDs deleted."
else
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    docker rm -f monitored-app-test-runner test-monitoring-runner >/dev/null 2>&1 || true
    docker rmi -f monitored-app:v1.0.0 >/dev/null 2>&1 || true
...
echo -e "\n${CLR_GREEN}${CLR_BOLD}✨ CLEANUP COMPLETE: All Monitoring resources successfully purged.${CLR_RESET}\n"
