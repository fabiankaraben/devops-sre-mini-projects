#!/usr/bin/env bash
# ==============================================================================
# test_monitoring_pipeline.sh - End-to-End Automated Test Suite for Mini-Project 17
# ==============================================================================
# Verifies:
#   1. Tool prerequisites (Docker, kubectl, Go)
#   2. Multi-stage Docker image build for monitored-app
#   3. Image footprint (< 25MB) and non-root UID 10001 security validation
#   4. Declarative Prometheus Operator CRDs & ServiceMonitors (verify_monitoring_stack.sh)
#   5. Alert evaluation & synthetic traffic simulation (alert_test_generator.sh)
...
cleanup_e2e() {
    local pids
    pids=$(pgrep -f "port-forward.*(monitored-app|prometheus)" 2>/dev/null || true)
    if [[ -n "$pids" ]]; then
...
echo "======================================================================"
echo "  🚀 Prometheus Operator & ServiceMonitor End-to-End Test Suite"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [2/6] Building Monitored App Container Image...${CLR_RESET}"
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    if docker build -t monitored-app:v1.0.0 "${SCRIPT_DIR}/app" > "$TMP_DIR/docker_build.log" 2>&1; then
        record_test "03" "Docker multi-stage build (monitored-app:v1.0.0)" 0 "Built successfully"
    else
        record_test "03" "Docker multi-stage build" 1 "Build failed, check $TMP_DIR/docker_build.log"
    fi

    IMG_SIZE_MB=$(docker image inspect monitored-app:v1.0.0 --format='{{.Size}}' 2>/dev/null | awk '{printf "%.2f", $1/1024/1024}' || echo "0")
    ...
    USER_ID=$(docker image inspect monitored-app:v1.0.0 --format='{{.Config.User}}' 2>/dev/null || echo "")
...
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [3/6] Running Prometheus Operator Policy Validation...${CLR_RESET}"
if "${SCRIPT_DIR}/verify_monitoring_stack.sh"; then
    record_test "06" "Prometheus Operator validation script (verify_monitoring_stack.sh)" 0 "All checks passed"
else
    record_test "06" "Prometheus Operator validation script" 1 "Validation checks failed"
fi

# ------------------------------------------------------------------------------
# Test 4: Run Alert Evaluation & Traffic Simulation
# ------------------------------------------------------------------------------
echo -e "\n${CLR_YELLOW}▶ [4/6] Running Alert Evaluation & Traffic Simulation...${CLR_RESET}"
if "${SCRIPT_DIR}/alert_test_generator.sh"; then
    record_test "07" "Alert evaluation and traffic simulation (alert_test_generator.sh)" 0 "Alert test passed"
else
    record_test "07" "Alert evaluation test" 1 "Alert test encountered errors"
fi
...
if "${SCRIPT_DIR}/cleanup.sh" >/dev/null 2>&1; then
    record_test "08" "Cleanup script execution (cleanup.sh)" 0 "All monitoring resources purged"
else
...
if [[ "$FAILED_TESTS" -eq 0 ]]; then
    echo -e "  ${CLR_GREEN}${CLR_BOLD}🎉 ALL MONITORING PIPELINE TESTS PASSED (${PASSED_TESTS}/${TOTAL_TESTS})${CLR_RESET}"
    echo "======================================================================"
    exit 0
else
