#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Cleanup Script for Mini-Project 01
# ==============================================================================
...
echo "======================================================================"
echo "  🧹 Cleaning Up Stateless App Mini-Project Resources"
echo "======================================================================"
echo -e "${CLR_RESET}"
...
PIDS=$(pgrep -f "port-forward.*stateless-app" || true)
if [[ -n "$PIDS" ]]; then
...
if kubectl get namespace "$NAMESPACE" >/dev/null 2>&1; then
    echo "  Deleting namespace ${NAMESPACE} and associated pods/services/deployments..."
...
# Remove standalone test containers if any
docker rm -f stateless-app-test test-v1 test-v2 test-stateless-runner >/dev/null 2>&1 || true

# Remove Docker images
docker rmi -f stateless-app:v1.0.0 stateless-app:v2.0.0 >/dev/null 2>&1 || true
...
find "$SCRIPT_DIR" -maxdepth 2 -type f \( -name ".rollout_results_*" -o -name ".tmp_*" -o -name "*.log" \) -exec rm -f {} +
