#!/usr/bin/env bash
# ==============================================================================
# cleanup.sh - Teardown and Cleanup Script for Mini-Project 05
# ==============================================================================
# Purges all Kubernetes resources, namespaces, HPA objects, Deployments,
# load generator pods, local Docker images, and temporary files.
# ==============================================================================
...
# 2. Delete Load Generator Pod
echo -e "\n${CLR_YELLOW}▶ [2/4] Deleting load generator pods...${CLR_RESET}"
if kubectl get namespace "$NAMESPACE" >/dev/null 2>&1; then
    kubectl delete pod hpa-load-generator -n "$NAMESPACE" --ignore-not-found=true --now >/dev/null 2>&1 || true
    echo -e "  [${CLR_GREEN}OK${CLR_RESET}] Load generator pods removed."
fi
...
docker rm -f hpa-test-runner >/dev/null 2>&1 || true
docker rmi -f autoscale-app:v1.0.0 >/dev/null 2>&1 || true
...
echo -e "\n${CLR_GREEN}${CLR_BOLD}✨ CLEANUP COMPLETE: All HPA & autoscaling resources successfully purged.${CLR_RESET}\n"
